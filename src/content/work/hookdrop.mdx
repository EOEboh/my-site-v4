---
title: 'hookdrop: webhook debugging SaaS, 49 users at launch with zero paid marketing'
outcome: 'Webhook inspection and replay SaaS, shipped solo: 49 users at launch with zero paid marketing.'
...
liveUrl: 'https://hookdrop.app'
repoUrl: 'https://github.com/EOEboh/hookdrop'
heroImage: '/images/work/hookdrop-img.png'
heroImageAlt: 'hookdrop dashboard streaming incoming webhook deliveries live, each with source, event type, and verified signature'
featured: true
flagship: true
order: 1
---
...
**Why:** the stream is one-way: the server announces new deliveries, the browser never pushes. SSE is plain HTTP: it works through proxies, reconnects natively via `Last-Event-ID`, and needs no upgrade handshake or ping-pong plumbing.
...
hookdrop replays requests from its own server. If your service runs on `localhost:3000` during development, the replay can't reach it: outbound requests from a VPS don't route into your laptop. Today the workaround is a staging URL or any tunnel you already have. The fix is a small CLI that opens a tunnel and receives replays locally, and it's next on the roadmap. I'd rather tell you now than let you find it mid-debug.
